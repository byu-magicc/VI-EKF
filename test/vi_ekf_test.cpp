...
//  est.register_mocap_cb(mocap_cb);
  ...
  // Log initial truth
  ofstream truth_log;
  truth_log.open("/tmp/multirotor_true_state.log");
  truth_log.write((char*)&multirotor.t_, sizeof(double));
  truth_log.write((char*)multirotor.state().arr.data(), multirotor.state().arr.rows() * sizeof(double));
