...
for line in meas_file:
    ...
    if meas_type == 'ACC':
        h.store(t, acc_hat=line_arr[0:2], acc_meas=line_arr[2:4])
    elif meas_type == 'ATT':
        if line_arr.shape == (9,):
            h.store(t, att_hat=line_arr[0:4], att_meas=line_arr[4:8])
    elif meas_type == 'POS':
        h.store(t, pos_hat=line_arr[0:3], pos_meas=line_arr[3:6])
    elif meas_type == 'FEAT':
        id = line_arr[4]
        h.store(t, line_arr[4], feat_hat=line_arr[0:2], feat_meas=line_arr[2:4])
        ids.append(id) if id not in ids else None
    elif meas_type == 'DEPTH':
        h.store(t, line_arr[2], depth_hat=line_arr[0], depth_meas=line_arr[1])
    else:
...
plot_side_by_side('x_pos', 0, 3, h.t.xhat, h.xhat, cov=None, truth_t=h.t.pos_meas, truth=h.pos_meas, labels=['x', 'y', 'z'], start_t=start, end_t=end)
plot_side_by_side('x_vel', 3, 6, h.t.xhat, h.xhat, cov=None, labels=['x', 'y', 'z'], start_t=start, end_t=end)
plot_side_by_side('x_att', 6, 10, h.t.xhat, h.xhat, cov=None, truth_t=h.t.att_meas, truth=h.att_meas, labels=['w','x', 'y', 'z'], start_t=start, end_t=end)

for i in ids:
    plot_side_by_side('lambda/x_{}'.format(i), 0, 2, h.t.feat_hat[i], h.feat_hat[i], truth_t=h.t.feat_meas[i], truth=h.feat_meas[i], labels=['u', 'v'], start_t=start, end_t=end)
    plot_side_by_side('rho/x_{}'.format(i), 0, 1, h.t.depth_hat[i], h.depth_hat[i], truth_t=h.t.depth_meas[i], truth=h.depth_meas[i], labels=[r'$\frac{1}{\rho}$'], start_t=start, end_t=end)
