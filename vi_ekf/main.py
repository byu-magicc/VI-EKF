...
# initialize features
for zeta, depth in zip(data[0][7], data[0][8]):
    ekf.init_feature(zeta, depth)
...
history = zip(*[[np.array(d) for d in instance] for instance in history])
...
if True:
    ...
    plot_side_by_side('mu', viekf.xMU, viekf.xMU+1, time, all_x_hat, cov=all_P, truth_t=None, truth=None, labels=['mu'])
