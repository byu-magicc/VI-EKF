  ...
  it_(nh_)
{
  ...
  odom_truth_sub_ = nh.subscribe("multirotor/truth/NED", 10, &VIEKF_ROS::odom_truth_callback, this);
...
void VIEKF_ROS::odom_truth_callback(const nav_msgs::OdometryConstPtr &msg) {
  ...
  z_att_ << msg->post.pose.orientation.w, msg->pose.pose.orientation.x, msg->pose.pose.orientation.y, msg->pose.pose.orientation.z;
  truth_callback(z_pos_, z_att_, msg->header.stamp);
}
